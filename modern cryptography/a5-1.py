...
def stepSix(plainText,keyStream): return "".join([str(xor(keyStream[i%228],plainText[i])) for i in range(len(plainText))])

def stepSeven(cipherText, keyStream): 
    return "".join([str(xor(keyStream[i%228],cipherText[i])) for i in range(len(cipherText))])
...
if __name__ == "__main__":
	header()
	plainText = padding228(input("Msg> "))
	print(plainText)
	# Step One #
	print ("\nInitializing LFSR...")
	lfsrs = stepOne()
    ...
    #string = "".join([str(xor(keyStream[i%228],cipherText[i])) for i in range(len(cipherText))])
    # decode = ""
    # for i range(len(cipherText)):
    #     decode = decode + str(xor(keyStream[i%228], cipherText[i]))
	print (cipherText,"\n\n")
